...
💡 **왜 DB에서는 이진트리가 아닌 다원 탐색 트리, 그 중 B-tree를 많이 사용하는가?**
이진 트리를 사용하면 n개의 원소에서 특정 값을 검색하는데에 O(log n)이므로 매우 빠르긴 하다. 하지만, **현대 컴퓨터에서는 산술, 논리 연산보다는 메모리 접근 연산이 훨씬 많은 시간을 소모한다.** 
<br>
기존의 **이진 탐색 트리**가 담고 있는 노드의 수가 100억 개라고 생각해보자. 그렇다면 모든 노드들을 메인 메모리(RAM)에 올려서 사용할수가 없으니, 결국 디스크(보조 기억 장치)에 탐색 트리를 두고 검색을 해야 한다. 운영체제를 공부하다 보면 알다시피 **디스크 접근은 메인 메모리 접근에 비해 훨씬 느리다**. 따라서 **디스크 접근을 최소화해야 빠른 시간에 검색을 마칠 수 있다**. (디스크에서 데이터를 읽거나 쓰기 위해서는 블록 단위로 접근해야 한다. *블록 : 디스크 공간을 구분하는 단위이며 고정된 값)
<br>
**다원 탐색 트리**란 이렇듯 기존의 BST에서 데이터 접근 과정에서 오는 낭비를 최소화 하고자 한 노드의 데이터를 여러개를 저장할 수 있도록 degree를 늘인 형태이다. 이때 이, 다원 탐색 트리에서도 마찬가지로 level을 최소화하면 탐색 성능이 더욱 향상할 수 있는데 이를 구현한 것이 **Btree**이다.
<br>
...
<br>
이러한 이유들로 데이터베이스 시스템은 대부분 balanced search tree인 B-tree 혹은 B+tree를 사용한다.
{: .notice}
